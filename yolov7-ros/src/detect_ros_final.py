...
class Yolov7Publisher:
    ...
    def process_img_msg(self, img_msg: Image):
        ...
        if self.visualization_publisher:
            ...
            # print("class num ::: ", classes)
            # rospy.loginfo(int(classes))
            # print(classes)
            #################################3
            ...
            if len(classes) > 0 :
                ...
                else : self.traffic_publisher.publish(False)

                if(10 in classes or 11 in classes or 12 in classes):
                    self.emergency_publisher.publish(True)
                else : self.emergency_publisher.publish(False)
            
                # if(11 in classes or 12 in classes):
                #     self.emergency_publisher.publish(True)
                # else : self.emergency_publisher.publish(False)
            else:
                self.traffic_publisher.publish(False)
